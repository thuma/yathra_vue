...
def StopIdToName(id):
  for row in stops.execute("SELECT agency_stop_id FROM astops WHERE stop_id = %s and agency_id = 269" % id):
    return str(row[0])
...
def search():
    ...
    result = requests.get(
        "https://reseplanerare.fskab.se/vasternorrland/v2/querypage_adv.aspx",
        data = data
        )
    trips = [1]
    products = []
    ...
    names = [
        ...
        "Vuxen periodkort",
        "Barn periodkort",
        "Ungdom/Senior periodkort",
        "Vuxen länskort",
        "Barn länskort",
        "Ungdom/Senior länskort",
        "",
        "",
        "",
        "",
    ...
    for trip in trips:
        ...
        for i, price in enumerate(prices):
          if itravelCats[search["travellersPerCategory"][0]["cat"]] in names[i]:
            ...
            url = "https://www.dintur.se/kopa-biljetter-och-reskort/"
            pricelist.append({
                "productId": url,
...
run(host='127.0.0.1', port=8102, reloader=True)