...
def StopIdToName(id):
  for row in stops.execute("SELECT agency_stop_id FROM astops WHERE stop_id = %s and agency_id = 271" % id):
    return str(row[0])
...
def search():
    ...
    result = requests.get(
        "http://www.reseplaneraren.lanstrafiken-z.se/v3/resultspage.aspx",
        data = data
        )
    trips = [1]
    products = []
    ...
    for trip in trips:
        ...
        for i, price in enumerate(prices):
          if itravelCats[search["travellersPerCategory"][0]["cat"]] in names[i]:
            ...
            url = "https://ltr.se/biljetter-priser/"
            rkp = round(pris*0.9)
            rkvat = round(rkp*0.06,2)
            pricelist.append({
                "productId": url,
...
run(host='127.0.0.1', port=8103, reloader=True)