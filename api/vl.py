...
def StopIdToName(id):
  for gtfsdata in stops.execute("SELECT stop_lon, stop_lat FROM stops WHERE stop_id = %s" % id):
    dist = 99999.0;
    for row in stops.execute("SELECT stop_lon, stop_lat, agency_stop_id, agency_stop_name FROM akstops WHERE stop_lon BETWEEN %s AND %s AND stop_lat BETWEEN %s AND %s AND agency_id = 266" % (gtfsdata[0]-0.5,gtfsdata[0]+0.5,gtfsdata[1]-0.5,gtfsdata[1]+0.5)):
...
def search():
    ...
    print result.content.split("priceArr[")
    for trip in trips:
        pricelist = []
