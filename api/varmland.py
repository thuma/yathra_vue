...
def StopIdToName(id):
  for row in stops.execute("SELECT agency_stop_id FROM astops WHERE stop_id = %s and agency_id = 265" % id):
    return str(row[0])
...
def search():
    ...
    result = requests.get(
        "https://reseplanerare.fskab.se/kollplatsen/v2/resultspage.aspx",
        data = data
        )
    trips = [1]
    products = []
